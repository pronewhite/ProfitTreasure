  ...
  <div>
    ...
    <div class="content clearfix">
      <div class="detail-left">
        <div class="detail-left-title">新手宝（{{ productInfo.productNo }}期）</div>
        <ul class="detail-left-number">
          ...
          <li>
            ...
            <span v-if="productInfo.productStatus == 0 ">募集中&nbsp;&nbsp;剩余募集金额{{productInfo.leftProductMoney}}元</span>
            <span v-else-if="productInfo.productStatus == 1">已满标</span>
            <span v-else-if="productInfo.productStatus == 2">满标且已经生成受益计划</span>
          </li>
          <li>
            <span>投资周期</span>
        ...
        <div class="datail-record">
          ...
          <div class="datail-record-list">
            <table align="center" width="880" border="0" cellspacing="0" cellpadding="0">
              ...
              <tbody>
              <tr v-for="(bid,index) in bidList" :key="bid.phone">
                <td>{{ index + 1 }}</td>
      ...
      <div class="detail-right">
        ...
        <div class="detail-right-mode">
          ...
          <form action="" id="number_submit">
            <p>请在下方输入投资金额</p>
            <input type="text" placeholder="请输入日投资金额，应为100元整倍数" v-model="investMoney" @blur="checkInvestMoney"  name="" class="number-money">
...
export default {
  ...
  methods: {
    ...
    initPage() {
      ...
      doGet('/v1/prodInvest/info', {ProductId: productId})
          .then(resp => {
            if (resp) {
      ...
      //查询资金
      doGet('/v1/user/center').then(resp => {
    ...
    checkInvestMoney() {
      if (isNaN(this.investMoney)) {
        ...
        if (this.productInfo.productType == 0) {
          incomeMoney = this.investMoney * this.productInfo.cycle * dayRate;
        } else {
          incomeMoney = this.investMoney * (this.productInfo.cycle * 30) * dayRate;
        }
      ...
      if (userinfo) {
        ...
        if (userinfo.name != '') {
          ...
          if (this.investMoneyErr == '') {
            doPost('/v1/invest/product', {proId: this.productInfo.id, money: this.investMoney})
                .then(resp => {
                  if (resp && resp.data.code == 1000) {
                    //投资成功
                    this.initPage();
